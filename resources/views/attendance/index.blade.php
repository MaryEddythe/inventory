...
<div class="bg-white rounded-4 shadow-sm p-4">
                    ...
                    <select name="employee_id" class="form-select" required>
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->emp_no }}" @selected(old('employee_id') == $employee->emp_no)>
                                {{ $employee->full_name }} @if($employee->division?->description) - {{ $employee->division->description }}@endif
                            </option>
                        @endforeach
                    </select>
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-white">
            <div class="fw-semibold">Records for {{ \Carbon\Carbon::parse($selectedDate)->format('F d, Y') }}</div>
        </div>
                ...
                <tbody>
                    @forelse($recordsForDate as $record)
                        <tr>
                            ...
                            <td>{{ $record->employee?->division?->description ?? 'N/A' }}</td>
                            <td>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <div class="fw-semibold">Monthly Late and Absence Tracker</div>
            <div class="text-muted small">Month range: {{ $monthStart->format('M d, Y') }} to {{ $monthEnd->format('M d, Y') }}</div>
        </div>
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                ...
                <tbody>
                    @forelse($employeeRows as $row)
                        <tr>
                            ...
                            <td>{{ $row['employee']->division?->description ?? 'N/A' }}</td>
